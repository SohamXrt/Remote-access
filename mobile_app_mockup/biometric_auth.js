...
class BiometricAuth {
    ...
    async isBiometricAvailable() {
        ...
        try {
            ...
            return {
                supported: available,
                reason: available ? 'Biometric authentication available' : 'No biometric sensors found'
            };
        } catch (error) {
            return { supported: false, reason: 'Cannot detect biometric capability' };
        }
    }
    ...
    // Authenticate using biometric (with fallback)
    async authenticateBiometric() {
        if (!this.isSupported) {
            // Fallback: simulate authentication with user confirmation
    ...
    // Fallback authentication for devices without WebAuthn
    async fallbackAuthentication() {
        return new Promise((resolve, reject) => {
            // Show confirmation dialog
            const confirmed = confirm(
                'Biometric authentication not available.\n\n' +
                'Click OK to confirm your identity and authenticate, or Cancel to abort.'
            );
